...
- Here exp(β1) = `r round(exp(M1.fish$coefficients[2]), 2)`, indicating that adjusting for year and length, the intensity of parasite in area2 is `r round(exp(M1.fish$coefficients[2]), 2)` times of that in area1.

- exp(β2) = `r round(exp(M1.fish$coefficients[3]), 2)`, indicating that adjusting for year and length, the intensity of parasite in area3 is `r round(exp(M1.fish$coefficients[3]), 2)` times of that in area1.

- exp(β3) = `r round(exp(M1.fish$coefficients[4]), 2)`, indicating that adjusting for year and length, the intensity of parasite in area4 is `r round(exp(M1.fish$coefficients[4]), 2)` times of that in area1.

- exp(β4) = `r round(exp(M1.fish$coefficients[5]), 2)`, indicating that adjusting for area and length, the intensity of parasite in year 2000 is `r round(exp(M1.fish$coefficients[5]), 2)` times of that in year 1999.

- exp(β5) = `r round(exp(M1.fish$coefficients[6]), 2)`, indicating that adjusting for area and length, the intensity of parasite in year 2001 is `r round(exp(M1.fish$coefficients[6]), 2)` times of that in year 1999.

- exp(β6) = `r round(exp(M1.fish$coefficients[7]), 2)`, indicating that adjusting for area and year, with one unit increase in length, the intensity of parasite would decrease by `r round(exp(M1.fish$coefficients[7]), 2)` times.
...
The Pearson's Chi-square test statistic is `r G.stat.fish`, deviance test statistic is `r D.stat.fish`. The p-values of both of them are less than 0.05 under 1184 df, indicating the fit is not good here.
